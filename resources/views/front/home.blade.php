...
<section id="feature_news_section" class="feature_news_section">
 ...
 <img class="img-responsive top_static_article_img" src="{{ asset('uploads/post') }}/{{ $hot_news->main_image }}"  alt="{{ $hot_news->title }}">
                    </div>
                    <!-- feature_article_img -->
 ...
 <div class="feature_article_date">
    <a href="{{ url('/author') }}/{{ $hot_news->user->id }}" >{{ $hot_news->user->name }}</a>,
      {{ date('F j,Y',strtotime( $hot_news->created_at )) }}           </a></div>
            ...
            <div class="col-md-5" style="margin-bottom: 3%">
                <div class="feature_static_wrapper">
                    <div class="feature_article_img">
                        <img class="img-responsive" width="450" style="height: 270px" src="{{ asset('uploads/post') }}/{{ $item->main_image }}" alt="{{ $item->title }}">
                    </div>
                    <!-- feature_article_img -->
                    ...
                    <div class="feature_article_inner">
                        ...
                        <div class="feature_article_title">
 <h1><a href="{{ url('/details') }}/{{ $item->slug }}">{{  $item->title  }} </a></h1>
                        </div>
                        <!-- feature_article_title -->

  <div class="feature_article_date"><a href="{{ url('/author') }}/{{ $item->user->id }}"  >{{ $item->user->name }}</a>, {{ date('F j,Y',strtotime( $item->created_at )) }}   </div>
                        <!-- feature_article_date -->
...
<div class="category_section mobile">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="top_article_img">
                    <a href="{{ url('/details') }}/{{ $item->slug }}"><img width="400px" class="img-responsive"
          src="{{ asset('uploads/post') }}/{{ $item->list_image }}" alt="{{ $item->title }}">
                    </a>
                </div>
                <!----top_article_img------>
            </div>
  ...
  <div class="category_article_date"> <a href="{{ url('/author') }}/{{ $item->user->id }}"  >{{ $item->user->name }}</a>, {{ date('F j,Y',strtotime( $item->created_at )) }}</div>
            ...
            <div class="col-md-6" style="margin-bottom: 2%">
                <div class="media">
                    <div class="media-left">
                        <a href="{{ url('/details') }}/{{ $item->slug }}"><img width="200px" class="media-object" src="{{ asset('uploads/post') }}/{{ $item->thumb_image }}"
                                         alt="{{ $item->title }}"></a>
                    </div>
                    <div class="media-body">
                        ...
                        <span class="media-date"><a href="{{ url('/author') }}/{{ $item->user->id }}"  >{{ $item->user->name }}</a>, {{ date('F j,Y',strtotime( $item->created_at )) }}</span>
...
<div class="widget m30">
    ...
    <div class="media">
        <div class="media-left">
            <a href="{{ url('/details') }}/{{ $item->slug }}"><img width="200px" class="media-object" src="{{ asset('uploads/post') }}/{{ $item->thumb_image  }}" alt="{{ $item->title }}"></a>
        </div>
        <div class="media-body">
            <h3 class="media-heading">
                <a href="{{ url('/details') }}/{{ $item->slug }}" >{{ $item->title }}</a>
